...
## Практические рекомендации
	Помимо описанных в соглашении правил существуют рекомендации по написанию кода, выработанные практикующими программистами, зачастую связанные с особенностями языка и среды исполнения:
	...
	- не игнорируйте исключения. Даже если Вы оставляете блок catch пустым, опищите в комментарии причину такого действия;
	...
	На практие алгоритм будет выглядеть примерно так:
	  1. Делаем Clone удалённого репозитория со всеми ветками;
		...
		5. Делаем Pull той ветки, от которой ответвились (проверяем наличие обновлений);
		  5.1. В случае, если в удалённую (родительскую) ветку были добавлены коммиты необходимо зайти в свою локальную ветку с изменениями;
		  5.2. Сделать Merge с родительской веткой (притянуть удалённые изменения к себе, разрешить возможные конфликты, перепроверить работоспособность);
...
_(Не нужно писать тесты, если вы всегда пишете код без ошибок, обладаете идеальной памятью и даром предвидения. Ваш код настолько крут, что изменяет себя сам, вслед за требованиями клиента, а также иногда код объясняет клиенту, что его требования — +гов+ не нужно реализовывать)_
	...
	Для написания юнит-тестов для языка Java существуют библиотеки JUnit, TestNG,
JavaTESK. Наиболее распространённым инструментом является JUnit.
...
* TODO Механизм тестирования
# Unit или нагрузочное?

* TODO Автоматизация и Jenkins
# Вообще куба-студия сама деплоит все изменения в томкэт, надо посмотреть как

* TODO [#C] Вопросы Гуглу
надо ли?
